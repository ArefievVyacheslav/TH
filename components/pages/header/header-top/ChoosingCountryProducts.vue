<template lang="pug">
  .pos-rel
    .df.aic.cp(@click="isShowDropdown = !isShowDropdown")
      component(:is="getIconLanguage(currentLanguage)" :style="'border-radius: 50%'")
      .language.ml6px.hover-text-dark(:class="{ 'text-dark': isShowDropdown }") {{ currentLanguage.reduction }}

    .pos-abs.t25px(v-if="isShowDropdown" :class="{ r205px: $mq === 'md' }")
      DropDown(@close="isShowDropdown = false" :styles="{ padding: '8px 12px 12px 12px', display: 'inline-block', right: `${ $mq === 'sm' ? '0' : false }` }")
        .df.aic
          span.language-header.toe.wsn.oh Выбор страны доставки
          div(@click.stop)
            //IconTooltip#tooltip-dropdown-language.ml6px.pt3px
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('filters', [ 'SET_FIND_PARAM', 'RESET_FILTERS' ]),
    ...
    getCountry () {
      this.languages.forEach(langObj => {
        if (this.$route.path.includes('/--' + langObj.reduction + '--')) {
          this.SET_FIND_PARAM({ param: 'country', value: langObj.country })

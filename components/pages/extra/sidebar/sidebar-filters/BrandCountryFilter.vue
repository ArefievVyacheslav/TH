    ...
    .extra-filters__choices(v-if="isShowBrandCountryFilter")
      ...
      .extra-filters__options-wrapper
        .filter__options-item.filter__options-item-subcategory.df.aic.cp(
          ...
          .df.aic.pl10px
            Checkbox(:is-checked="currentBrandCountry.includes(brandCountryStr)" styles="top: 4px; left: 3px;")
...
export default {
  ...
  watch: {
    ...
    '$store.state.filters.findObj.brandCountry': {
      handler (nV) {
        if (!nV) this.currentBrandCountry = []
      }
    }
  ...
  methods: {
    ...
    hideFilters () {
      this.isShowBrandCountryFilter = false
    },
    setFindParam () {
      this.SET_FIND_PARAM({
        param: 'brandCountry',
        value: { $in: this.currentBrandCountry.concat() }
      })
    },
    unsetFindParam () {
  ...
  mounted () {
    ...
    setTimeout(() => this.getBrandCountry(), 1500)
  }
};
</script>
